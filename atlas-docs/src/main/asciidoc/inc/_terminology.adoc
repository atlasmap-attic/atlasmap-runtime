...
|Data Source
|The definition of an source or target data format within an AtlasMapping file
...
|Field Action
|A function to perform on a given value of an source or target field
...
|Lookup Table
|A cross reference table used at runtime to define a data conversion that varies based on the value of the source field
|Used for mapping enumerations

|Mapping
|A grouping of source Field(s), target Field(s) and optionally Field Action(s) that defines the conversion to be performed at runtime within an AtlasMapping file
...
|Validation
|A Validation is a syntax check of a provide mapping file to ensure execution may proceed
